...
def check_res(r):
    ...
    # sleep for a second every time we check a response to
    # throttle rate. Otherwise it's easy to get put in github jail
    time.sleep(1)
...
OUTPUT: retrieved issues sorted by their number if request was successful. False otherwise
'''
def download_issues(source_url, source, credentials, state):
    page = 1
    issues = []
    while True:
        url = source_url+"repos/"+source+"/issues?page="+str(page)
        print(url)
        r = get_req(url, credentials)
        status = check_res(r)
...
OUTPUT: Null
'''
def create_issues(issues, destination_url, destination, milestones, labels, milestone_map, credentials, sameInstall, assignee):
...
def main():
    ...
    parser.add_argument('--assignee', '-a', nargs='?', type=str, help='The user to assign issues to if you are moving issues.')
    ...
    if args.issues:
        ...
        if issues:
            ...
            if (args.sourceRoot == args.destinationRoot):
                sameInstall = True
            res = create_issues(issues, destination_root, destination_repo, args.milestones, args.labels, milestone_map, destination_credentials, sameInstall, args.assignee)
        elif issues == False:
